...
def conectar():
    caminho = os.path.join(os.path.dirname(os.path.abspath(__file__)), "caixa.db")
...
def inserir_pagamento(cpf, codigo_pagamento, parcela, valor, competencia):
    ...
    cursor.execute("""
    INSERT OR IGNORE INTO pagamentos (cpf, codigo_pagamento, parcela, valor, competencia, status)
    VALUES (?, ?, ?, ?, ?, ?)
